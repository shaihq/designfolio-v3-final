...
export async function registerRoutes(app: Express): Promise<Server> {
  app.post("/api/convert-resume", upload.single("resume"), async (req: MulterRequest, res) => {
    try {
      ...
      const prompt = `Based on the following resume text, extract and generate professional portfolio content. You MUST return the data in the following JSON format: { "user": { "name": "Full Name", "role": "A concise, impactful one-liner that describes who the person is and their professional essence (e.g., 'Passionate about building products that solve real-world problems' or 'Transforming complex data into actionable business strategies'). DO NOT use a simple job title or role name here.", "aboutMe": "A friendly, humane 2-3 sentence introduction about the person's interests and professional philosophy.", "categories": ["Skill Category 1", "Skill Category 2", "Skill Category 3", "Skill Category 4", "Skill Category 5", "Skill Category 6"], "skills": [ {"name": "Skill Name", "level": 0-100} ], "contact": { "email": "Email address", "phone": "Phone number or null", "location": "City, Country or null" } }, "workExperiences": [ { "role": "Job Title", "company": "Company Name", "period": "Years (e.g. 2020 - 2023)", "description": "Short achievement" } ], "caseStudies": [ { "title": "Project Title", "description": "Project overview", "category": "Project Category" } ] } CRITICAL: 1. For the "categories" array, you MUST provide at least 6 professional skills. 2. The "aboutMe" section should be written in first person and sound natural. 3. For the footer, extract contact details. If not found, use realistic placeholders. Resume Text: ${text};`;
